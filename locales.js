export default {
  en: {
    links: {
      home: "Home",
      projects: "Portfolio",
      contacts: "Contacts",
      about: "About this website",
    },
    buttons: {
      about_me: "About me",
      projects: "Portfolio",
      contacts: "Contacts",
    },
    welcome: {
      name: "Sergey Shapochkin",
      description: "Programmer, musician, 3d artist.",
      about: {
        title: "About me",
        skills: {
          title: "Skills",
        },
        projects: {
          title: "Portfolio",
          description:
            "Some websites, apps and REST APIs. A lot of pet-projects, random scripts and configs. Addons and Python modules for Blender (3d modelling suite).",
          link: "Take a look!",
        },
      },
    },
    about: {
      title: "About this website",
      description:
        "This website was built using Vue 2, Vuex, Nuxt.js, Bootstrap, vue-i18n and hosted on GitHub Pages",
    },
    contacts: {
      title: "Contacts",
    },
    projects: {
      title: "My personal projects",
      description: "Some open source pieces of software i that worked on.",
      search: "Search projects",
      tags: "Tags",
      project: {
        site_link: "Site",
        source_code_link: "GitHub",
        video_link: "Video",
      },
    },
    skills: {
      general: {
        name: "General",
        content:
          "GNU/Linux (ubuntu, debian, fedora, arch), bash, git, vim, vscode, regex",
      },
      virtualization: {
        name: "Virtualization",
        content: "Docker, libvirt, virt-manager, VirtualBox",
      },
      web: {
        name: "Web",
        content:
          "Django, DRF, Vue 2/3, Vuex, Pinia, Bootstrap 5, Vuetify, vue-i18n, sass, TCP/IP basics",
      },
      python: {
        name: "Python 3",
        content: "numpy, pandas, matplotlib",
      },
      _3d: {
        name: "3d",
        content:
          "Blender, AutoCAD, Unity3D render pipelines, PBR in general, hard-surface modelling, non-destructive modelling",
      },
      languages: {
        name: "Languages",
        content: "Russian, English, Japanese",
      },
    },
    projects_data: {
      alprou: "Website for managing your routines.",
      alprou_backend: "Website for managing your routines. (backend)",
      portfolio: "This website.",
      emtk: "Blender addon, designed to simplify editing modifiers stack through modal operators and abstraction layers.",
      libemtk: "Python library for Blender, designed to bring new level of abstraction to modifiers stack.",
      modal_shortcuts: "Keyboard shortcuts and other utility for modal operators in Blender",
      class_variables_editor_ui: "UI for editing python's class variables in Blender",
    },
    other_projects_data: {
        name: "Other projects",
        description: "My GitHub repositories."
    },
  },
  ru: {
    links: {
      home: "Главная страница",
      projects: "Портфолио",
      contacts: "Контакты",
      about: "Об этом сайте",
    },
    buttons: {
      about_me: "Обо мне",
      projects: "Портфолио",
      contacts: "Контакты",
    },
    welcome: {
      name: "Шапочкин Сергей",
      description: "Программист, музыкант, 3d моделлер.",
      about: {
        title: "Обо мне",
        skills: {
          title: "Навыки",
        },
        projects: {
          title: "Портфолио",
          description:
            "Несколько сайтов, веб-приложений и REST API. Pet-проекты, различные скрипты и конфиги. Аддоны и Python-модули для Blender (софт для работы с 3D графикой).",
          link: "Просмотреть",
        },
      },
    },
    about: {
      title: "Об этом сайте",
      description:
        "Этот сайт был сделан с использованием Vue 2, Vuex, Nuxt.js, Bootsrap 5, vue-i18n и размещен на GitHub Pages.",
    },
    contacts: {
      title: "Контакты",
    },
    projects: {
      title: "Мои проекты",
      description: "Некоторые из моих open source проектов.",
      search: "Поиск по проектам и тэгам",
      tags: "Тэги",
      project: {
        site_link: "Сайт",
        source_code_link: "GitHub",
        video_link: "Видео",
      },
    },
    skills: {
      general: {
        name: "Общие",
      },
      web: {
        name: "Веб-разработка",
      },
      virtualization: {
        name: "Виртуализация",
      },
      _3d: {
        name: "3D",
        content:
          "Blender, AutoCAD, Unity3D render pipelines, PBR в целом, hard-surface modelling, non-destructive modelling",
      },
      languages: {
        name: "Языки",
        content: "Русский, английский, японский",
      },
    },
    projects_data: {
        alprou: "Веб-приложение, предназначеное для использования в качестве трекера привычек.",
        alprou_backend: "Сервер Alprou. Использует Django REST framwork и несколько других модулей для Django.",
        portfolio: "Этот сайт.",
        emtk: "Аддон для Blender, разработан для того чтобы упростить редактирование стэка модификаторов с помощью модальных операторов и слоев абстракции.",
        libemtk: "Python-модуль для Blender. Сериализация/десериализация рекурсивных структур данных, множественное наследование/mixin-классы, кэширование и модульное тестирование.",
        modal_shortcuts: "Клавиатурные сокращения и другой функционал для модальных операторов в Blender.",
        class_variables_editor_ui: "UI для редактирования классовых переменных Python в Blender",
    },
    other_projects_data: {
        name: "Другие проекты",
        description: "Мои репозитории на GitHub."
    },
  },
};
